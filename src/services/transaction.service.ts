...
const SELECT_WITH_BOOKMAKER =
  '*, bookmaker:bookmakers(id, name, slug, min_deposit, commission_value)';
...
export const transactionService = {
  ...
  /**
   * Cria uma movimentação. O afiliado informa o titular da conta aberta pelo
   * link (ele mesmo ou outra pessoa); o registro entra como "em análise" até
   * o administrador validar.
   */
  async create(userId: string, input: CreateTransactionInput): Promise<Transaction> {
    // Garante que a casa existe (e está acessível) antes de gravar.
    await bookmakerService.findById(input.bookmaker_id);
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        account_holder_is_self: input.account_holder_is_self,
        account_holder_name: input.account_holder_name,
        account_holder_cpf: input.account_holder_cpf,
    ...
    if (error) throw new AppError(`Falha ao registrar movimentação: ${error.message}`, 500);
