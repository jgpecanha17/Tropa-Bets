...
export function DashboardView({
  ...
  return (
    <>
          ...
          <div>
            ...
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {Number(active.min_deposit) > 0 ? (
                <span className="inline-flex items-center rounded-full bg-amber-400/10 px-2.5 py-1 text-xs font-semibold text-amber-300">
                  Aporte mínimo: {formatCurrency(Number(active.min_deposit))}
                </span>
              ) : null}
              {Number(active.commission_value) > 0 ? (
                <span className="inline-flex items-center rounded-full bg-lime/10 px-2.5 py-1 text-xs font-semibold text-lime">
                  Comissão por conta: {formatCurrency(Number(active.commission_value))}
                </span>
              ) : null}
            </div>
          </div>
      ...
      <TransactionForm bookmaker={active} profile={profile} />
